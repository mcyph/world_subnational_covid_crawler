bikou_map = {
    # Kyoto
    '京都市': 'kyoto-shi',
    '京都府': 'kyoto-shi',
    '京都府内': 'kyoto-shi',
    '右京区役所勤務': 'kyoto-shi',

    '園児の保護者でPCR検査を実施、陽性となった': 'kyoto-shi',
    'ナイル川クルーズ船ツアー参加': 'kyoto-shi',
    '3/16オランダから帰国': 'kyoto-shi',
    '3/15フランス、ルクセンブルク経由で帰国': 'kyoto-shi',
    '3/15ルクセンブルグから帰国': 'kyoto-shi',
    'キルギス': 'kyoto-shi',
    '外国籍': 'other',

    '丹後': 'tango-kan',
    '丹後管内': 'tango-kan',
    '丹後保健所管内': 'tango-kan',

    '中丹管内': 'nakatan-kan',
    '中丹': 'nakatan-kan',
    '中丹西保健所管内': 'nakatan-kan',
    '中丹東保健所': 'nakatan-kan',
    '中丹東保健所管内': 'nakatan-kan',
    '中丹西': 'nakatan-kan',
    '中丹東': 'nakatan-kan',

    '南丹管内': 'nantan-kan',
    '南丹保健所': 'nantan-kan',
    '南丹保健所管内': 'nantan-kan',
    '南丹': 'nantan-kan',

    '山城管内': 'yamashiro-kan',
    '山城': 'yamashiro-kan',
    '山城北保健所': 'yamashiro-kan',
    '山城北保健所管内': 'yamashiro-kan',
    '山城南保健所管内': 'yamashiro-kan',
    '山城北保健所管内在住': 'yamashiro-kan',
    '山城南保健所館内': 'yamashiro-kan',
    '再陽性/山城北保健所管内': 'yamashiro-kan',
    '山城南保健所': 'yamashiro-kan',
    '山城市内': 'yamashiro-kan',
    '山城北': 'yamashiro-kan',
    '山城南': 'yamashiro-kan',

    '乙訓管内': 'otokuni-kan',
    '乙訓保健所': 'otokuni-kan',
    '乙訓保健所管内': 'otokuni-kan',
    '乙訓': 'otokuni-kan',
    '乙君': 'otokuni-kan',

    # Okinawa
    '南部保健所管内': '南部保健所管',
    '3/19スペイン（マドリード州）から帰国/成田空港検疫所/中部保健所管内在住': '中部保健所管',
    '3/4ベルギーから帰国/中部保健所管内': '中部保健所管',
    '中部保健所管内': '中部保健所管',
    '中部保健所': '中部保健所管',
    '南部保健所': '南部保健所管',
    '北部保健所': '北部保健所管',
    '八重山保健所': '八重山保健所管',
    '八重山保健所管内': '八重山保健所管',
    '北部保健所管内': '北部保健所管',

    # Osaka
    '3/11イタリア（ピエモンテ州）から帰国/関西空港検疫所': '',

    # Fukuoka
    '糟屋郡/3/18イギリス（ロンドン）から帰国': '糟屋郡',
    '京都郡': '京都郡',
    '糟屋郡': '糟屋郡',
    '検疫官': 'unknown',
    '八女郡': '八女郡',
    '帰省先：岩国市': '岩国市',
    '遠賀郡': '遠賀郡',
    '嘉穂郡': '嘉穂郡',
    '城南区': '城南区',
    '東区': '東区',
    '博多区': '博多区',
    '西区': '西区',
    '早良区': '早良区',
    '中央区': '中央区',
    'ボリビア': 'other',
    '田川郡': '田川郡',
    '三井郡': '三井郡',
    '諫早市': '諫早市',
    '三潴郡': '三潴郡',
    '朝倉郡': '朝倉郡',
    '糸島市': '糸島市',
    '鞍手郡': '鞍手郡',

    # Kanagawa
    '鎌倉保健所管内': '鎌倉保健所管',
    '厚木保健福祉事務所': '厚木保健福祉事務所管',
    '厚木保健福祉事務所管内': '厚木保健福祉事務所管',
    '横浜検疫所職員': 'other',
    '鎌倉保健福祉事務所': '鎌倉保健福祉事務所管',
    '平塚保健福祉事務所管内': '平塚保健福祉事務所管',
    '小田原保健福祉事務所管内': '小田原保健福祉事務所管',
    '鎌倉保健福祉事務所管内': '鎌倉保健福祉事務所管',
    '茅ヶ崎保健所管内/鎌倉市内のスーパーに勤務': '茅ヶ崎保健所管内',
    '平塚保健福祉事務所管内/フランス帰国者': '平塚保健福祉事務所管',
    '3/8イタリア（ヴェネト州）から帰国/東京検疫所東京空港（羽田空港）検疫所支所': '',
    '3/16アメリカ（ニューヨーク）から帰国/平塚保健福祉事務所管内': '平塚保健福祉事務所管',
    '茅ヶ崎市保健所管内': '茅ヶ崎市保健所管',
    '運転免許合宿': 'unknown',
    '平塚保健福祉事務所': '平塚保健福祉事務所管',
    '小田原保健福祉事務所': '小田原保健福祉事務所管',
    'ブラジル': 'other',
    'ペルー、メキシコ': 'other',
    'アメリカ合衆国': 'other',
    '行動歴：アメリカ合衆国': 'other',
    '行動歴：アラブ首長国連邦': 'other',
    '行動歴：パキスタン': 'other',
    '行動歴：フィリピン': 'other',
    '行動歴：非公表': 'unknown',
    '行動歴：サウジアラビア、イギリス': 'other',
    '行動歴：ペルー': 'other',
    '行動歴：バングラデシュ': 'other',
    '行動歴：イギリス': 'other',
    '行動歴：メキシコ': 'other',
    '行動歴：カメルーン': 'other',
    '行動歴：ブラジル': 'other',
    '行動歴：インド': 'other',
    '行動歴：スペイン': 'other',
'行動歴：トルコ': 'other',
'行動歴：タイ': 'other',
'行動歴：ベトナム': 'other',
'行動歴：ドイツ': 'other',
'行動歴：イタリア': 'other',
'行動歴：インドネシア': 'other',
'行動歴：中国': 'other',
'行動歴：ネパール': 'other',
'行動歴：フランス': 'other',
'行動歴：韓国': 'other',
'行動歴：オランダ': 'other',
'行動歴：アフガニスタン': 'other',
'行動歴：ロシア': 'other',
'行動歴：ポルトガル': 'other',
'行動歴：マルタ、フランス': 'other',
'行動歴：台湾': 'other',
'行動歴：ウクライナ': 'other',
'行動歴：ニュージーランド': 'other',
'行動歴：アフガニスタン、アラブ首長国連邦': 'other',
'行動歴：タジキスタン': 'other',
'行動歴：ウズベキスタン': 'other',
'※データサイトから収集、再精査必要': 'unknown',
'行動歴：スウェーデン': 'other',
'※「確定日」列には発表日が入っている': 'unknown',
'※「確定日」列には発表日が入っている 宝塚健康福祉事務所管内': '宝塚健康福祉事務所管',
'※「確定日」列には発表日が入っている 伊丹健康福祉事務所管内': '伊丹健康福祉事務所管',
'※「確定日」列には発表日が入っている 龍野健康福祉事務所管内': '龍野健康福祉事務所管',
'行動歴：ルーマニア': 'other',
'※「確定日」列には発表日が入っている 加東健康福祉事務所管内': '加東健康福祉事務所管',
'※「確定日」列には発表日が入っている 洲本健康福祉事務所管内': '洲本健康福祉事務所管',
'行動歴：スロバキア': 'other',
'※「確定日」列には発表日が入っている 加古川健康福祉事務所管内': '加古川健康福祉事務所管',
'行動歴：モルドバ': 'other',

'※「確定日」列には発表日が入っている 赤穂健康福祉事務所管内': '赤穂健康福祉事務所管',
'行動歴：ハンガリー': 'other',
'行動歴：ミャンマー': 'other',
'行動歴：スイス': 'other',
'行動歴：チェコ': 'other',
'※「確定日」列には発表日が入っている 豊岡健康福祉事務所管内': '豊岡健康福祉事務所管',
'※「確定日」列には発表日が入っている 中播磨健康福祉事務所管内': '中播磨健康福祉事務所管',
'※「確定日」列には発表日が入っている 朝来健康福祉事務所管内': '朝来健康福祉事務所管',
'※「確定日」列には発表日が入っている 丹波健康福祉事務所管内': '丹波健康福祉事務所管',
'行動歴：バングラディシュ': 'other',


    '中原区': '中原区',

    # Aichi
    '尾張地方': '尾張地方',
    '三河地方': '三河地方',
    '一宮保健所管内': '一宮保健所管',
    '3/13スペイン（マドリード州）から帰国/東京検疫所東京空港（羽田空港）検疫所支所': '',
    'フィリピン': 'other',
    'パキスタン': 'other',
    '愛知県内': 'other',  # TODO!

    # Tochigi
    '県南健康福祉センター管内/ダイヤモンド・プリンセス号2/19下船': '県南健康福祉センター管',
    '県南健康福祉センター管内': '県南健康福祉センター管',
    '3/15ポルトガルから帰国/県南健康福祉センター管内': '県南健康福祉センター管',
    '安足健康福祉センター': '安足健康福祉センター管',
    '県西健康福祉センター': '県西健康福祉センター管',
    '県南健康福祉センター': '県南健康福祉センター管',
    '県南保健福祉センター管内': '県南保健福祉センター管',
    'アラブ首長国連邦': 'other',

    # Kochi
    '安芸福祉保健所管内': '安芸福祉保健所管',
    '中央東福祉保健所管内': '中央東福祉保健所管',
    '中央西福祉保健所管内': '中央西福祉保健所管',
    '幡多福祉保健所管内': '幡多福祉保健所管',
    '須崎福祉保健所管内': '須崎福祉保健所管',
    '中央東': '中央東福祉保健所管',
    '中央西': '中央西福祉保健所管',
    'ペルー': 'other',

    # Mie
    '3/4ベトナムから帰国/名古屋検疫所中部空港検疫所支所': '',
    '伊賀保健所管内': '伊賀保健所管',
    'ダイヤモンド・プリンセス号を下船': 'unknown',

    # Yamanashi
    '南アルプス市〜昭和町が生活圏': '南アルプス市',
    '生活圏：南アルプス市、中央市': '中央市',
    '生活圏：南アルプス市、甲斐市': '甲斐市',
    '生活圏：市川三郷町、富士川町': '富士川町',
    '生活圏：甲府市、富士川町': '富士川町',
    '生活圏：中央市': '中央市',
    '生活圏：甲府市、甲州市': '甲州市',
    '生活圏：甲州市': '甲州市',
    '生活圏：南アルプス市': '南アルプス市',
    '生活圏：南アルプス市、笛吹市': '笛吹市',
    '生活圏：甲斐市、昭和町': '昭和町',
    '生活圏：韮崎市、北杜市': '北杜市',
    '生活圏：上野原市': '上野原市',
    '生活圏：甲府市、甲斐市': '甲斐市',
    '再陽性': 'unknown',
    '生活圏：甲斐市': '甲斐市',
    'イギリス': 'other',
    '生活圏：北杜市、昭和町': '北杜市',
    '生活圏：富士吉田市、富士河口湖町': '富士吉田市',
    '生活圏：中北地域、峡南地域': '峡南地域',
    '生活圏：見延町': '見延町',
    '甲府市': '甲府市',
    '甲府市、南アルプス市': '甲府市',
    '東部地域': '東部地域',
    '甲府市外（中北地域）': '中北地域',
    '非公表': 'unknown',
    '昭和町': '昭和町',
    '南アルプス市': '南アルプス市',
    '中北地域': '中北地域',
    '生活圏：富士北麓地域': '富士北麓地域',
    '生活圏：峡東地域': '峡東地域',
    '生活圏:山梨市、甲州市': '甲州市',
    '生活圏:甲府市、甲斐市': '甲斐市',
    '生活圏:富士北麓地域': '富士北麓地域',

    # Nara
    '郡山保健所管内、本来陽性確定日は９日だが、厚生労働省のとりまとめが10日だったため、便宜上10日にしている': '郡山保健所管',
    '3/16メキシコから帰国': 'unknown',
    '中和保健所管内': '中和保健所管',
    '郡山保健所管内': '郡山保健所管',
    '3/29アメリカより帰国/郡山保健所管内': '郡山保健所管',

    # Hyogo
    '伊丹保健所管内': '伊丹保健所管',
    '宝塚保健福祉事務所管内': '宝塚保健福祉事務所管',
    '伊丹保健福祉事務所管内': '伊丹保健福祉事務所管',
    '加東健康福祉事務所管内': '加東健康福祉事務所管',
    '伊丹健康福祉事務所管内': '伊丹健康福祉事務所管',
    '伊丹健康福祉事務所管内/「グリーンアルス伊丹」の利用者': '伊丹健康福祉事務所管',
    '宝塚健康福祉事務所管内': '宝塚健康福祉事務所管',
    '洲本健康福祉事務所管内': '洲本健康福祉事務所管',
    '芦屋健高福祉事務所管内': '芦屋健高福祉事務所管',
    '芦屋健康福祉事務所管内': '芦屋健康福祉事務所管',
    '宝塚健康保健福祉事務所管内': '宝塚健康保健福祉事務所管',
    '宝塚健康福祉事務所館内': '宝塚健康福祉事務所管',
    '丹波健康福祉事務所管内': '丹波健康福祉事務所管',
    '赤穂健康福祉事務所管内': '赤穂健康福祉事務所管',
    '*': 'unknown',
    '赤穂': '赤穂',
    '加古川健康福祉事務所管内': '加古川健康福祉事務所管',
    '龍野健康福祉事務所管内': '龍野健康福祉事務所管',
    '豊岡健康福祉事務所管内': '豊岡健康福祉事務所管',
    '中播磨健康福祉事務所管内': '中播磨健康福祉事務所管',

    # Nagano
    '松本保健所管内': '松本保健所管',
    '松本保健所管内/厚労省NO140の家族': '松本保健所管',
    '佐久保健所管内': '佐久保健所管',
    '飯田保健所管内': '飯田保健所管',
    '大町保健所管内': '大町保健所管',
    '諏訪保健所管内': '諏訪保健所管',
    '伊那保健所管内': '伊那保健所管',
    '木曽保健所管内': '木曽保健所管',
    '北信保健所管内': '北信保健所管',
    'シンガポール': 'other',

    # Kumamoto
    '御船保健所管内': '御船保健所管',
    '阿蘇保健所管内': '阿蘇保健所管',
    '有明保健所管内': '有明保健所管',
    '阿蘇郡市': '阿蘇郡市',
    '天草保健所管内': '天草保健所管',
    '有明保健所': '有明保健所管',
    '菊池保健所': '菊池保健所管',
    '八代保健所': '八代保健所管',

    # Ibaraki
    '東庄町所在の社会福祉施設': '',
    '職員家族': 'unknown',
    '古河保健所管内': '古河保健所管',
    '龍ケ崎保健所管内': '龍ケ崎保健所管',
    'つくば保健所管内': 'つくば保健所管',
    '中央保健所管内': '中央保健所管',
    '筑西保健所': '筑西保健所管',

    # Aomori
    '3/20アメリカから帰国': 'unknown',
    '上十三保健所': '上十三保健所管',
    '上十三保健所管内': '上十三保健所管',
    '郡山市保健所管内': '郡山市保健所管',
    '五所川原保健所管内': '五所川原保健所管',
    '五所川原保健所': '五所川原保健所管',
    '弘前保健所': '弘前保健所管',

    # Shizuoka
    '3/22イタリア（ロンバルディア州）から帰国': 'unknown',
    '榛原郡': '榛原郡',
    '賀茂郡': '賀茂郡',
    '西部保健所管内': '西部保健所管',
    '東部保健所管内': '東部保健所管',
    '浜松市23〜52は1人を除きすべて浜松市在住': 'unknown',  # CHECK ME!
    '西部保健所': '西部保健所管',
    '富士保健所管内': '富士保健所管',
    '御殿場保健所管内': '御殿場保健所管',
    '熱海保健所管内': '熱海保健所管',

    # Fukushima
    'いわき市保健所管内': 'いわき市保健所管',
    '県北保健所管内': '県北保健所管',
    '築上郡': '築上郡',
    'インドネシア': 'other',
    '県南保健所': '県南保健所管',
    '県南': '県南保健所管',
    'いわき市': 'いわき市保健所管',

    # Ehime
    '東予地域': '東予地域',

    # Tokushima
    'ダイヤモンド・プリンセス号2/20下船': 'unknown',
    '徳島保健所管内': '徳島保健所管',
    '徳島': '徳島保健所管',
    '阿南': '阿南保健所管',
    '徳島/100歳以上': '徳島保健所管',
    '徳島保健所': '徳島保健所管',
    '阿南保健所': '阿南保健所管',

    # Akita
    '由利本荘保健所管内/外国籍': '由利本荘保健所管',
    '由利本荘保健所管内': '由利本荘保健所管',
    '湯沢保健所管内': '湯沢保健所管',
    '能代保健所': '能代保健所管',
    '由利本荘保健所': '由利本荘保健所管',
    '大仙保健所管内': '大仙保健所管',
    '秋田中央保健所管内': '秋田中央保健所管',

    # Gunma
    '渋川\n保健所管内': '保健所管',
    '桐生保健所': '桐生保健所管',
    '桐生保健所管内': '桐生保健所管',
    '館林\n保健所管内': '保健所管',
    '館林保健所管内': '館林保健所管',
    '帰省先が佐久市': 'other',
    '桐生\n保健所管内': '保健所管',
    '富岡保健所管内': '富岡保健所管',
    '伊勢崎保健所管内': '伊勢崎保健所管',
    '吾妻保健所管内': '吾妻保健所管',
    '利根沼田保健所管内': '利根沼田保健所管',
    '渋川保健所管内': '渋川保健所管',
}
